...
For any $c_1 < c_2$ and constants $a,b,c > 0$: 
 ...
 - $O(n^{c_1} < O(n^{c_2}$ 
 - $O(a) \cup O(\log n) \cup O(n^b) \cup O(c^n)$
 - multiplying above by positive values still hold so: $O(n) \cup O(n \log n)$ for example.
...
utility of big-Oh. Also for now, I have excluded discussion of multivariate functions and
