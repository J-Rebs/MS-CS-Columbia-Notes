...
define expected value as $E[X] = \sum_{x \in U} x \cdot Pr[X=x]$.
...
$X_1, \ldots, X_k$ have $E[\sum_{i=1}^k X_k] = \sum_{i=1}^k E[X_i]$.
...
Using linearity, $E[X] = E[\sum_{i=1}^k I_i] = \sum_{i=1}^k E[I_i] = \sum_{i=1}^k (1/2) = k/2$

<Callout emoji="💭">
I think understanding expected values and linearity can be confusing, but one way I am trying
to think about it is in terms of experiments. If I have experiment A that produces blue slime and
experiment B that produces orange slime. I can ask, if I run experiment A with experiment B, how much blue and orange
slime do I expect to have? On some level, this does not seem very intuitive. If experiment A and experiment B are separate,
then how can running one tell me anything about the other does? Each one will simply do what they do (even if there is some correlation between them). In fact, with linearity, I can say something that makes more sense.
If I want to know how much blue and orange slime I have after running experiment A and experiment B, I can
instead just run experiment A and experiment B separately adding their results. Note this is equivalent to saying: E[A + B]. 
If you have E[AB]...then things can get more complicated depending on correlation between A and B.
See [here](https://math.stackexchange.com/questions/2080030/linearity-of-expectations-why-does-it-hold-intuitively-even-when-the-r-v-s-are).
</Callout>